...
function animate_twosite() {
  let height = 500;
  let svgwidth = d3.select(".article").node().getBoundingClientRect().width;
  let width = svgwidth;
  let animationstate = 0;
  let hposSphere = width / 2;
  ...
  svg.append("g")
    .attr("transform", `translate(${npolecoords[0] - 20}, ${npolecoords[1] - 50}) scale(1.3)`)
  ...
  svg.append("g")
    .attr("transform", `translate(${spolecoords[0] - 110}, ${spolecoords[1] + 20}) scale(1.3)`)
    .append(() => mjx.querySelector("svg"))
    .attr("color", "#000000")
  ...
  svg.append("g")
    ...
    .append("path")
    .attr("d", path4)
    .attr("fill", d => (d.type == "LineString" ? "none" : "#000000"))
    .attr("stroke", "#000000")
    .attr("stroke-width", 2)
